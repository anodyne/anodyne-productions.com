...
class AddonList extends Component
{
    ...
    public function addons()
    {
        return Addon::query()
            ...
            ->when(
                $this->user,
                fn (Builder $query, User $user) => $query->where(
                    'user_id',
                    $user->id
                )
            )
            ->when(
                $this->filters['types'],
                fn (Builder $query, array $values): Builder => $query->whereIn(
                    'type',
                    $values
                )
            )
            ...
            ->when(
                $this->filters['rating'],
                fn (Builder $query, string $rating) => $query->where(
                    'rating',
                    '>=',
                    $rating
                )
    ...
    protected function setFilterDefaults(): void
    {
        $this->filters['compat_status'] = [
            CompatibilityStatus::Compatible
                ->value => CompatibilityStatus::Compatible->getLabel(),
            CompatibilityStatus::Incompatible
                ->value => CompatibilityStatus::Incompatible->getLabel(),
            CompatibilityStatus::Unknown
                ->value => CompatibilityStatus::Unknown->getLabel(),
        ];
