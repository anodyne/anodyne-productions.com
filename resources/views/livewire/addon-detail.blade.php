    ...
    <div class="lg:grid lg:grid-cols-7 lg:grid-rows-1 lg:gap-x-8 lg:gap-y-10 xl:gap-x-16">
        ...
        <div class="lg:col-span-4 lg:row-end-1" x-data="{ image: 0 }">
            <div class="aspect-w-4 aspect-h-3 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-800">
                @if ($addon->getMedia('primary-preview')->count() === 1)
                    <img
                        src="{{ $addon->getFirstTemporaryUrl(now()->addMinutes(5), 'primary-preview') }}"
                        alt=""
                    ...
                    >
                @else
                    <div class="bg-slate-200 dark:bg-slate-800 flex items-center justify-center ring-1 ring-inset ring-slate-950/5 dark:ring-white/5">
                ...
                @if ($addon->getMedia('additional-previews')->count() > 0)
                    @foreach ($addon->getMedia('additional-previews') as $preview)
                        <img
                            src="{{ $preview->getTemporaryUrl(now()->addMinutes(5)) }}"
                            alt=""
                            class="object-cover object-left"
                            x-show="image === {{ $loop->iteration }}"
                            x-cloak
                        >
                    @endforeach
                @endif
                ...
                <div class="grid grid-cols-5 gap-6 mt-6 px-1.5">
                    ...
                    @foreach ($addon->getMedia('additional-previews') as $preview)
                        <button
                            ...
                            x-on:click="image = {{ $loop->iteration }}"
                        >
        ...
        <div class="mx-auto mt-14 max-w-2xl sm:mt-16 lg:col-span-3 lg:row-span-2 lg:row-end-2 lg:mt-0 lg:max-w-none">
                    ...
                    <div class="flex items-center space-x-1.5 relative mt-2">
                        ...
                        @if ($versions->count() > 1)
                            <x-dropdown class="w-40">
                                ...
                                <x-dropdown.group>
                                    @foreach ($versions as $addonVersion)
                ...
                <div class="mt-6">
                    @if ($version->releaseSeries()->count() === 1)
                        <x-badge color="sky" size="xs">
                            Requires {{ $version->releaseSeries()->first()->name }}
                        </x-badge>
                    @endif

                    @if ($version->releaseSeries()->count() > 1)
                        <x-badge color="sky" size="xs">
                            Requires {{ $version->releaseSeries()->orderBy('name', 'asc')->first()->name }}+
                        </x-badge>
                    @endif
                </div>
            @endif
            ...
            <a href="{{ route('addons.index', $addon->user) }}" class="mt-6 flex items-center space-x-3">
                <div class="squircle">
                    <img src="{{ $addon->user->getFirstMediaUrl('avatar') }}" alt="" class="size-12">
                </div>
                ...
                <div class="mt-10 border-t border-slate-200 dark:border-slate-200/10 pt-10">
                    ...
                    <div class="mt-4 text-slate-600 dark:text-slate-400">
                        <dl class="space-y-6" role="list">
                            @foreach ($releaseSeries as $series)
                                @php
                                            ...
                                            <dt class="flex flex-1 items-center">
                                                ...
                                                <div aria-hidden="true" class="ml-1 flex flex-1 items-center">
                                                    ...
                                                    <div class="relative ml-3 flex-1">
                                                        ...
                                                        @php($rating = $reviewStats->{"rating$r"} / $addon->reviews->count())
