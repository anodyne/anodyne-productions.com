...
>
    ...
    <x-slot:sidebar>
        <div class="space-y-8">
            <div>
                <x-input.text placeholder="Search for add-ons..." id="search" wire:model.live.debounce.500ms="filters.search">
                    ...
                    @foreach ($this->products as $id => $name)
                        <li class="relative flex items-center">
                            <x-input.checkbox
                                :label="$name"
                                id="product_{{ $id }}"
                                wire:model.live="filters.products"
                                :value="$id"
                            ></x-input.checkbox>
                        </li>
                    @endforeach
    ...
    <div class="block lg:hidden mb-8" x-data="{ open: false }">
        ...
        <div class="mt-6 grid grid-cols-1 gap-6" x-show="open" x-cloak x-collapse>
            <div>
                <x-input.text placeholder="Search for add-ons..." id="mobile_search" wire:model.live.debounce.500ms="filters.search">
                    @if (filled($filters['search']))
                        <x-slot:trailingAddOn>
                            <button wire:click="$set('filters.search', '')">
                                @svg('flex-delete-circle', 'h-4 w-4')
                ...
                <div class="relative mt-3">
                    <dl class="space-y-1">
                        @for ($r = 4; $r >= 1; $r--)
                            ...
                            >
                                <input type="radio" wire:model.live="filters.rating" id="mobile_rating{{ $r }}" value="{{ $r }}" class="hidden">
    ...
    >
        @forelse ($this->addons as $addon)
            <a href="{{ route('addons.show', [$addon->user, $addon]) }}" class="flex flex-col bg-white dark:bg-slate-800 overflow-hidden shadow hover:shadow-lg dark:shadow-lg rounded-3xl ring-1 ring-slate-900/5 transition">
                <div class="relative px-0.5 pt-0.5">
                    @if ($addon->getMedia('primary-preview')->count() === 1)
                        <img src="{{ $addon->getFirstTemporaryUrl(now()->addMinutes(5), 'primary-preview') }}" alt="" class="w-full h-56 object-cover rounded-t-[calc(theme(borderRadius.3xl)-2px)] rounded-b-lg">
                    @else
                        <div class="bg-slate-200 dark:bg-slate-800 ring-1 ring-inset ring-slate-950/5 flex items-center justify-center px-6 py-16 rounded-t-[calc(theme(borderRadius.3xl)-2px)] rounded-b-lg">
                            <x-logos.nova.mark class="text-slate-400 dark:text-slate-600 h-24 w-auto" :colors="false"></x-logos.nova.mark>
                        </div>
                    @endif
                </div>

                <div class="relative flex-1 w-full px-4 sm:px-6">
                    <div>
                        <img src="{{ $addon->user->getFirstMediaUrl('avatar') }}" alt="" class="size-24 rounded-full absolute -translate-y-1/2 ring-[3px] ring-white dark:ring-slate-800">
                    </div>
                    ...
                    <div class="my-6 space-x-2.5 flex items-center">
