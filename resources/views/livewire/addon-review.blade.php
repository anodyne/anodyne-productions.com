<div class="flex flex-col h-full">
    <div class="p-4 sm:px-6 sm:py-5">
        ...
        <div class="mt-1 pl-7 text-xs text-slate-600 dark:text-slate-400">
            Rate and review the {{ $addon->name }} {{ str($addon->type->getLabel())->lower() }}
        </div>
    </div>
    ...
    <div class="overflow-y-auto px-4 pb-4 sm:px-6 sm:pb-4 prose lg:text-sm dark:prose-invert max-w-none space-y-6">
            ...
            <div class="flex items-center space-x-1">
                @for ($s = 1; $s <= 5; $s++)
                    <label
                        for="star{{ $s }}"
                        @class([
                            'text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400' => $rating < $s,
                            'text-amber-400 dark:text-amber-500' => $rating >= $s,
                        ])
                    >
                        <input wire:model.live="rating" type="radio" name="rating" id="star{{ $s }}" value="{{ $s }}" class="hidden">
        ...
        <x-input.group label="Review">
            <x-input.textarea wire:model.live="content"></x-input.textarea>
        </x-input.group>
    </div>
