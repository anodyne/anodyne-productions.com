...
function MobileNavigation({ items = [], user }) {
  return (
    <Popover>
      <Popover.Button
...
export function LandingNavigation({ items = [], dark = false }) {
  const { user } = useAuth()
...
export function AppNavigation({ items = [], dark = false }) {
  const { user } = useAuth()

  return (
    <nav className="sticky top-0 z-40 w-full backdrop-blur flex-none transition-colors duration-500 lg:z-50 lg:border-b lg:border-slate-900/10 dark:border-slate-50/[0.06] bg-white/75 supports-backdrop-blur:bg-white/60 dark:bg-transparent">
      <div className="max-w-8xl mx-auto">
        <div className="py-4 border-b border-slate-900/10 lg:px-8 lg:border-0 dark:border-slate-300/10 mx-4 lg:mx-0">
          <div className="relative flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center">
              <nav>
                <ul className="flex items-center text-sm font-medium gap-x-8">
                  {items.map((item) => (
                    <li key={item.href}>
                      <NavLink href={item.href} dark={dark}>{item.title}</NavLink>
                    </li>
              ...
              <div className="flex items-center border-l border-slate-200 ml-6 pl-6 dark:border-slate-800">
                ...
                {user ? (
                  ...
                  <Link href={`${process.env.NEXT_PUBLIC_BACKEND_URL}/login`} className="text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400 transition">
