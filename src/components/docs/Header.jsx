...
export const Header = forwardRef(function Header({ className, currentVersion, navigation }, ref) {
    ...
    >
      ...
      <div className="flex items-center gap-5 lg:hidden">
        ...
        <Link href="/" aria-label="Home">
          <NovaLogo className="h-6 text-slate-700 dark:text-white" />
        </Link>
      </div>
      <div className="flex items-center gap-5">
        ...
        <div className="hidden min-[416px]:contents">
          <Button href={process.env.NEXT_PUBLIC_BACKEND_URL}>Sign in</Button>
        </div>
      </div>
    </motion.div>
  )
